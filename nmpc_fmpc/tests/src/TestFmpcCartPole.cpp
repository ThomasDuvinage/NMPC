...
class TestFmpcCartPole : public rclcpp::Node
{
public:
  TestFmpcCartPole()
  : Node("TESTmpc_cart_pole")
  {
    // Setup ROS2 Publisher
    marker_arr_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("marker_arr", 1);

    constexpr double dist_force_small = 10.0; // [N]
    dist_left_small_srv_ = this->create_service<std_srvs::srv::Empty>(
        "/dist_left_small", std::bind(&TestFmpcCartPole::distCallback, this, std::placeholders::_1, std::placeholders::_2, -1 * dist_force_small));
    dist_right_small_srv_ = this->create_service<std_srvs::srv::Empty>(
        "/dist_right_small", std::bind(&TestFmpcCartPole::distCallback, this, std::placeholders::_1, std::placeholders::_2, dist_force_small));
    constexpr double dist_force_large = 30.0; // [N]
    dist_left_large_srv_ = this->create_service<std_srvs::srv::Empty>(
        "/dist_left_large", std::bind(&TestFmpcCartPole::distCallback, this, std::placeholders::_1, std::placeholders::_2, -1 * dist_force_large));
    dist_right_large_srv_ = this->create_service<std_srvs::srv::Empty>(
        "/dist_right_large", std::bind(&TestFmpcCartPole::distCallback, this, std::placeholders::_1, std::placeholders::_2, dist_force_large));
    target_pos_m5_srv_ = this->create_service<std_srvs::srv::Empty>(
        "/target_pos_m5", std::bind(&TestFmpcCartPole::targetPosCallback, this, std::placeholders::_1, std::placeholders::_2, -5.0));
    target_pos_0_srv_ = this->create_service<std_srvs::srv::Empty>(
        "/target_pos_0", std::bind(&TestFmpcCartPole::targetPosCallback, this, std::placeholders::_1, std::placeholders::_2, 0.0));
    target_pos_p5_srv_ = this->create_service<std_srvs::srv::Empty>(
        "/target_pos_p5", std::bind(&TestFmpcCartPole::targetPosCallback, this, std::placeholders::_1, std::placeholders::_2, 5.0));

    // Instantiate problem
    double horizon_dt = 0.01; // [sec]
    double horizon_duration = 2.0; // [sec]
    this->get_parameter("control/horizon_dt", horizon_dt);
    this->get_parameter("control/horizon_duration", horizon_duration);
    fmpc_problem_ = std::make_shared<FmpcProblemCartPole>(
        horizon_dt, std::bind(&TestFmpcCartPole::getRefPos, this, std::placeholders::_1));
    this->get_parameter("param/cart_mass", fmpc_problem_->param_.cart_mass);
    this->get_parameter("param/pole_mass", fmpc_problem_->param_.pole_mass);
    this->get_parameter("param/pole_length", fmpc_problem_->param_.pole_length);
    std::vector<double> param_vec;
    if (this->get_parameter("cost/running_x", param_vec))
    {
      fmpc_problem_->cost_weight_.running_x = Eigen::Map<FmpcProblemCartPole::StateDimVector>(param_vec.data());
    }
    if (this->get_parameter("cost/running_u", param_vec))
    {
      fmpc_problem_->cost_weight_.running_u = Eigen::Map<FmpcProblemCartPole::InputDimVector>(param_vec.data());
    }
    if (this->get_parameter("cost/terminal_x", param_vec))
    {
    ...
    mpc_timer_ = this->create_wall_timer(
      std::chrono::duration<double>(horizon_dt / 1e+9),
  ...
  void run()
  {
    ...
    this->get_parameter("control/mpc_dt", mpc_dt);
    this->get_parameter("control/sim_dt", sim_dt);
    current_t_ = 0;
    ...
    bool no_exit = false;
    this->get_parameter("no_exit", no_exit);
    ...
    while (rclcpp::ok() && (no_exit || current_t_ < end_t))
    {
      ...
      // Publish marker
      marker_arr_pub_->publish(this->makeMarkerArr());
...
protected:
  ...
  bool distCallback(const std::shared_ptr<std_srvs::srv::Empty::Request> /*req*/, std::shared_ptr<std_srvs::srv::Empty::Response> /*res*/, double dist_force)
  {
    dist_u_ << dist_force;
  ...
  bool targetPosCallback(const std::shared_ptr<std_srvs::srv::Empty::Request> /*req*/, std::shared_ptr<std_srvs::srv::Empty::Response> /*res*/, double pos)
  {
    target_pos_ = pos;
    return true;
  }

  visualization_msgs::msg::MarkerArray makeMarkerArr()
  {
    ...
    mass_marker.ns = "mass";
    mass_marker.id = static_cast<int>(marker_arr_msg.markers.size());
...
TEST(TestFmpcCartPole, SolveMpc)
{
  ...
  rclcpp::sleep_for(std::chrono::seconds(1));  // ROS2 sleep

  test.run();  // Run your test function
}
